...
Square::Square(float side, Settings const& settings) : Mesh(settings)
{
    ...
    for (int i = 0; i < m_resolution; i++)
    {
        ...
        {
            m_vertices[m_resolution * i + j].x = (1.f * i / (m_resolution - 1) - 0.5f) * side;
            m_vertices[m_resolution * i + j].y = (1.f * j / (m_resolution - 1) - 0.5f) * side;
